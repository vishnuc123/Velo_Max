      ...
      try {
        ...
        submitButton.addEventListener("click", async function () {
          ...
          submitFormButton.addEventListener("click",  function (event) {
            ...
            Swal.fire({
              ...
              if (result.isConfirmed) {
                try {
                  ...
                  const primaryImageInput = document.querySelector("input[name='coverImage']");
                  if (primaryImageInput && primaryImageInput.files.length > 0) {
                    ...
                    if (uploadedImageUrl) {
                      formData.append("primaryImage", uploadedImageUrl);
                    }
                  }
          
                  const additionalImageInputs = document.querySelectorAll("input[name^='additionalImage_']");
                  ...
                  if (additionalImageUrls.length > 0) {
                    formData.append("additionalImages", JSON.stringify(additionalImageUrls));
                  }
          
                 
          
                  // this function is for clean the form data ella binary data remove akkan
                  const keysToDelete = [];
            ...
            if (threedModelInput && threedModelInput.files.length > 0) {
              const uploadedModelUrl = await uploadToCloudinary(
                threedModelInput.files[0],
                "3d_models",
                "auto"
              );
              if (uploadedModelUrl) {
                formData.append("threedModel", uploadedModelUrl);
              }
            }
          
             
                  const response = await axios.post(`/product/Addproduct/${selectedCategory}`,formData,{
                    headers:{
...
function createThreedGroup(labelText,name) {
  ...
  input.type = "file";
  input.accept = ".glb,.gltf,.fbx,.obj"; // You can specify other 3D file formats
  input.name = name;
  input.className = "form-input w-full p-2 border border-gray-300 rounded";
  group.appendChild(input);

  // Create a <model-viewer> element to show the 3D model preview
  const modelViewer = document.createElement("model-viewer");
  modelViewer.className = "mt-2 w-full h-64 border border-gray-300 rounded";
  modelViewer.setAttribute("alt", "3D model preview");
  modelViewer.setAttribute("background-color", "transparent");
  modelViewer.setAttribute("camera-controls", "");
  modelViewer.setAttribute("auto-rotate", "");
  group.appendChild(modelViewer);
...
function openEditModal(product, category) {
  ...
  submitButton.addEventListener("click", async (event) => {
    ...
    // Create form data object to hold the JSON data
    const formData = {};
  
    // Handle cover image upload
    if (coverImageGroup.querySelector("input[type='file']").files.length > 0) {
      const file = coverImageGroup.querySelector("input[type='file']").files[0];
      const uploadedUrl = await uploadToCloudinary(file, "your-folder-name");
      if (uploadedUrl) {
        formData.coverImage = uploadedUrl;
      }
    } else {
      formData.coverImage = product.coverImage;  // Keep existing URL if no new image is selected
    }
    ...
    for (let input of additionalImageInputs) {
      if (input.files.length > 0) {
        ...
        const imageSrc = input.previousElementSibling ? input.previousElementSibling.src : "";
        if (imageSrc) {
          additionalImages.push(imageSrc);  // Keep existing URL if no new image is selected
        }
      }
    }
  
    if (additionalImages.length > 0) {
      formData.additionalImages = additionalImages;  // Send updated or existing image URLs
    }
  
    // Populate the rest of the fields with form data
    const inputs = form.querySelectorAll("input, textarea");
    inputs.forEach(input => {
      if (input.type !== "file") {
        formData[input.name] = input.value; // Set regular form field values
      }
    });
  ...
  function createDynamicFields(product, formContainer) {
    ...
    Object.keys(product).forEach((key) => {
      ...
      if (value === "true" || value === "false" || value === "on") {
        ...
        input.type = "number";
        input.className =
          "form-input w-full p-2 border border-gray-300 rounded";
        input.name = key;
        input.value = value;
  ...
  function validateForm(form) {
    ...
    const regexPatterns = {
      ...
      productBrand: /^[a-zA-Z0-9][a-zA-Z0-9 ]*$/,
      productRegularPrice: /^\d+(\.\d{1,2})?$/,
      productListingPrice: /^\d+(\.\d{1,2})?$/,
      productStock: /^[1-9]\d*$/,
    };
    ...
    inputs.forEach((input) => {
      ...
      if (regexPatterns[input.name]) {
        const regex = regexPatterns[input.name];
        if (!regex.test(input.value.trim())) {
          showError(input, `Please enter a valid ${input.name}.`);
          isValid = false;
          return;
        }
      }
      ...
      // Validate listing price is not less than regular price
      if (input.name === "productListingPrice") {
        const regularPrice = parseFloat(form.querySelector("input[name='productRegularPrice']").value);
        const listingPrice = parseFloat(input.value);
        if (listingPrice < regularPrice) {
          showError(input, "Listing price cannot be less than regular price.");
          isValid = false;
        }
